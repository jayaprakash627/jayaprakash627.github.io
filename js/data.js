...
const PORTFOLIO = {
  ...
  projects: [
    ...
    },
    {
      ...
      blurb: "Fleet management for 150 e-bikes: range estimated from each bike's own measured consumption, a ranked list of what needs a van today, and an explicit account of what the data can't measure. Real Capital Bikeshare demand.",
      tags: ["Backend", "REST API", "FastAPI", "SQLite", "GPS", "Data Quality"],
      accent: "tertiary",
      metric: "150 bikes · 7,600 trips",
      status: "Shipped",
      liveUrl: "",
      codeUrl: "https://github.com/jayaprakash627/ebike-fleet-telematics",
      featured: false,
      study: {
        problem: "A map with 400 dots on it is not an answer — the operator has one van and a morning. And the naive range figure, battery percent times advertised range, gives the same number for a bike ridden uphill by a heavy rider in February as for one ridden flat and light in June, and the same number on a two-year-old pack as on a new one. Bikes get stranded, or get collected while they were still useful.",
        built: "A FastAPI service over a fleet of 150 bikes and 7,600 trips that estimates each bike's range from what it has actually consumed — the median of its own trips in watt-hours per kilometre, with a 10th–90th percentile band — and folds in pack health, since a battery at 78% health holds 78% of the watt-hours it shipped with. On top of that: a ranked action list where a healthy bike appears nowhere, and a canvas map where a bike whose device has gone quiet is drawn hollow.",
        decisions: "Three rules, and they are all about what the tool is entitled to claim. A bike with fewer than five usable trips doesn't have a history, it has an anecdote — those fall back to the fleet median and the API says so in the field the UI renders, because silently substituting the fleet average is rarely wrong and unauditable when it is. A device silent for twelve hours suppresses all its own other findings, since showing 'low range' beside 'no contact for three days' invites acting on a number nobody can stand behind. And the fleet summary contains no averages at all — an average range is true of no bike and hides exactly the tail the operator is paid to care about. There's a test asserting no summary key contains 'avg' or 'mean'.",
        scope: "The trip demand is real — Capital Bikeshare's published history for April 2023, e-bikes only — so the daily rhythm and the way bikes accumulate where people leave them are measured, not invented. The battery telemetry is modelled, because no bike-share operator publishes it, and no amount of realism makes that the same thing. Distances are corrected straight lines rather than routed paths, and every distance in the API is labelled an estimate.",
        rebuilt: "The estimator could have been a circular argument, so it's graded instead. The simulator draws a hidden per-bike efficiency and writes only watt-hours and kilometres to the database; a test greps the app package to prove the estimator never reads the truth file. It recovers each bike's hidden value to within 0.44 Wh/km against a 9–16 spread, with no directional bias, and its band contains the true value for every bike. Two things I got wrong and fixed: the first device model marked a third of a healthy fleet as missing, because it conflated 'last trip' with 'last heartbeat' when the entire reason to fit a telematics unit is that it reports whether or not the bike moves. And the app held one SQLite connection in a module global, shared across FastAPI's threadpool — the dashboard fetches three endpoints at once, so three threads used it simultaneously. SQLite doesn't raise on that; statements interleave and a fetch comes back holding another query's values. It appeared as a type error on roughly one request in a handful and passed on retry, which is the worst way for a fault to present. Fixed with a per-thread connection rather than a lock, so reads still run in parallel, and pinned with a regression test I verified actually fails against the old code.",
        proves: "The most interesting number in the project is one I could have hidden: of 7,600 trips, 661 are real round trips whose distance is simply unrecoverable from start and end coordinates. They're not corrupt data — they're real rides this method cannot measure, which is a different sentence and the honest one. There's a whole endpoint and a whole screen for what the data can't tell you.",
      },
    },
    {
      title: "Battery State-of-Health Prediction",
      blurb: "Estimates how much capacity a pack has left from a single ordinary charge — no full discharge, no cycle history. Trained on NASA's cell-ageing data and scored on cells it had never seen.",
      tags: ["Python", "Machine Learning", "scikit-learn", "FastAPI", "Batteries", "Model Evaluation"],
      accent: "secondary",
      metric: "±1.63 pts · unseen cells",
      status: "Shipped",
      liveUrl: "",
      codeUrl: "https://github.com/jayaprakash627/battery-soh-prediction",
      featured: true,
      study: {
        problem: "Measuring a battery's true capacity means running it flat and counting amp-hours — hours of downtime, so nobody does it to a customer's e-bike. But every pack gets charged, and a charger already measures voltage, current and temperature. So: can you read a pack's health from an ordinary charge, knowing nothing about its history?",
        built: "A model that estimates state of health from one charge curve, using seven features a charger could compute on its own — charge accepted across a fixed voltage window, the incremental-capacity (dQ/dV) peak and where it sits, the constant-voltage tail, and the temperature rise. It lands within ±1.63 percentage points on cells it was never trained on, and ships as a FastAPI service with a demo that reads real measured curves and compares them against the full discharge that actually followed.",
        decisions: "The real work was the evaluation, not the model. Scored three ways: shuffled cycles gives MAE 1.23, leave-one-cell-out gives 1.63, early-life-to-late-life gives 1.70. The shuffled split is the one usually reported and it is close to meaningless — cycle 100 and cycle 101 of the same cell are the same measurement an hour apart, so a model scores well by interpolating between neighbours it has already seen. Ranked on that split you would ship gradient boosting; ranked on held-out cells it is the worst of the three candidates, flattered by 1.91 points against ridge's 0.39. Ridge ships, on a selection rule written into the code before the numbers were seen. Cycle number and previous capacity are excluded as features even though they are the strongest predictors in the dataset — they leak the answer, and a second-hand pack does not arrive with its cycle count written on it. There is a test that fails if either reappears.",
        scope: "Only 8 of the archive's 34 cells were run under a protocol where measured capacity means the same thing — the rest vary discharge rate, cut-off voltage or ambient temperature, which makes the label incomparable between cells. Every rejection is recorded with its reason. It reads a cell, not a pack; it was trained between 64% and 101% SoH and flags predictions outside that band; and it is not a safety system — it estimates capacity and says nothing about internal shorts or thermal runaway.",
        rebuilt: "Two things went wrong and both are in the repo with the reasoning attached. Three charges begin 30 mV under the measurement window, having been left part-charged, and cross it six times faster than the next cycle of the same cell at the same capacity — the cell had not aged, it just started nearly full. They were the model's three worst predictions: one cell's healthiest cycle, at 101.8% SoH, was read as 74.9%. Requiring a proper run-up before the window costs 3 cycles out of 902 and moved by-cell error from 1.74 to 1.63 and the early-to-late-life split from 3.11 to 1.70. Separately, two features turned out to be the same measurement twice — correlated at exactly 1.00, because inside constant current, charge is current times time.",
        proves: "I can tell the difference between a model that scores well and a model that works, and I would rather report the smaller honest number than the bigger one. The app explains every prediction as an exact per-feature breakdown — and states, on every prediction, where that explanation stops being trustworthy, because three of the features are correlated above 0.9 and the split of credit between them is not identified.",
      },
    },
    {
